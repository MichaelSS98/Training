...
import {View, StyleSheet, Modal, TextInput, Text, ScrollView} from 'react-native';
import { Button, IconButton } from 'react-native-paper';
import {Formik} from 'formik';
import { useMutation } from '@apollo/client';
import { GET_PROJECTS, UPDATE_PROJECT } from '../../../queries/Projects';
...
const ProjectUpdateModal = ({popupUpdate, setPopupUpdate, projectInfo}) => {
    ...
    const [updateProject] = useMutation(UPDATE_PROJECT, {
        update(cache, {data}) {
            ...
            const newProjects = existingProjects.getProjects.map(e => {
              if (e.id === projectId)
    ...
    const initialValues = {
        ...
        project_code: projectInfo[2],
        start_date: customDateTimeDisplay(new Date(projectInfo[3])),
        planned_end_date: customDateTimeDisplay(new Date(projectInfo[4])),
        description: projectInfo[5]
    };
